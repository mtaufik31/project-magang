<nav class="md:bg-transparent  font-poppins transition-all">
    <div class="max-w-screen-xl flex flex-wrap items-center justify-between mx-auto xl:p-4 py-4 px-6">
        <a href="{{ route('home') }}"
            class="flex items-center space-x-3 rtl:space-x-reverse hover:scale-105 duration-100 fill-black">
            <img width="180px" loading="lazy" src="{{ asset('asset/img/manga.png') }}" alt="">
        </a>
            ...
            <div class="hidden md:block">
                @auth
                    @php
                        ...
                        $coinBalance = auth()->user()->coinBalance->total_coins ?? 0;
                    @endphp

                    <!-- Panggil komponen CoinButton -->
                    <x-points :point="$coinBalance" />
                @endauth
            </div>
            <div class="hidden md:block">
